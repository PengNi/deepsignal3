...
class ModelExtraction(nn.Module):
    def __init__(self, seq_len=13, signal_len=16, num_layers1=3, num_layers2=1, num_classes=2,
                 ...
                 vocab_size=16, embedding_size=4, is_base=True, is_signallen=True,
                 is_trace=False,
                 module="both_bilstm", device=0,lambd=1.0):
        ...
        if self.module != "signal_bilstm":
            ...
            if self.is_base:
                ...
                if self.is_signallen and self.is_trace:
                    ...
                    out_seq = torch.cat((kmer_embed, base_means, base_stds, base_signal_lens,tags), 2)  # (N, L, C)
                elif self.is_trace:
                    out_seq = torch.cat((kmer_embed, base_means, base_stds, tags), 2)  # (N, L, C)
                else:
                    out_seq = torch.cat((kmer_embed, base_means, base_stds,tags), 2)  # (N, L, C)
            else:
                if self.is_signallen and self.is_trace:
                    out_seq = torch.cat((base_means, base_stds, base_signal_lens, tags), 2)  # (N, L, C)
                elif self.is_signallen:
                    out_seq = torch.cat((base_means, base_stds, base_signal_lens,tags), 2)  # (N, L, C)
                elif self.is_trace:
...
class ModelCNN(nn.Module):
    def __init__(self, seq_len=13, signal_len=16, num_layers1=3, num_layers2=1, num_classes=2,
                 dropout_rate=0.5, hidden_size=256,
                 vocab_size=16, embedding_size=4, is_base=True, is_signallen=True,
                 is_trace=False,module="both_bilstm",device=0):
    ...
    def forward(self, kmer, base_means, base_stds, base_signal_lens, signals):
        ...
        out_seq = torch.reshape(out_seq, (-1, self.nhid_seq * 2, self.hidden_size)).float()
        out_seq = self.fc_seq(out_seq)  # (N, L, nhid_seq)
        # out_seq = self.dropout_seq(out_seq)
        out_seq = self.relu_seq(out_seq)
        ...
        out_signal = torch.reshape(out_signal, (-1, self.nhid_signal * 2, self.hidden_size)).float()
        out_signal = self.fc_signal(out_signal)  # (N, L, nhid_signal)
        # out_signal = self.dropout_signal(out_signal)
        out_signal = self.relu_signal(out_signal)
...
class ModelCG(nn.Module):
    ...
    def forward(self, kmer, base_means, base_stds, base_signal_lens, signals,tags,cg_contents):
        # seq feature ============================================
        if self.module != "signal_bilstm":
            base_means = torch.reshape(base_means, (-1, self.seq_len, 1)).float()
            base_stds = torch.reshape(base_stds, (-1, self.seq_len, 1)).float()
            ...
            if self.is_base:
                kmer_embed = self.embed(kmer.long())
                if self.is_signallen and self.is_trace:
                    out_seq = torch.cat((kmer_embed, base_means, base_stds, base_signal_lens,tags,cg_contents), 2)  # (N, L, C)
                elif self.is_signallen:
                    out_seq = torch.cat((kmer_embed, base_means, base_stds, base_signal_lens,tags,cg_contents), 2)  # (N, L, C)
                elif self.is_trace:
                    out_seq = torch.cat((kmer_embed, base_means, base_stds,tags,cg_contents), 2)  # (N, L, C)
                else:
                    out_seq = torch.cat((kmer_embed, base_means, base_stds,tags,cg_contents), 2)  # (N, L, C)
            else:
                if self.is_signallen and self.is_trace:
                    out_seq = torch.cat((base_means, base_stds, base_signal_lens,tags,cg_contents), 2)  # (N, L, C)
                elif self.is_signallen:
                    out_seq = torch.cat((base_means, base_stds, base_signal_lens,tags,cg_contents), 2)  # (N, L, C)
                elif self.is_trace:
                    out_seq = torch.cat((base_means, base_stds,tags,cg_contents), 2)  # (N, L, C)
                else:
                    out_seq = torch.cat((base_means, base_stds,tags,cg_contents), 2)  # (N, L, C)
