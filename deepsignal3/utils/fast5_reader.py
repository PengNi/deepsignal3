...
class SingleFast5:
    ...
    def get_readid(self):
        ...
        if sys.version_info[0] >= 3:
            try:
                read_id = str(
                    self._read["/".join([reads_group_single, first_read])].attrs[
                        "read_id"
                    ],
                    "utf-8",
                )
            ...
            read_id = str(
                self._read["/".join([reads_group_single, first_read])].attrs["read_id"]
            )
        # print(read_id)
        return read_id
    ...
    def get_lastest_basecallgroup(self):
        groups = [
            y
            for y in filter(
                lambda x: x.startswith("Basecall_1D"), self._read["Analyses"].keys()
            )
        ]
    ...
    def get_fastq(
        self, basecall_group="Basecall_1D_000", basecall_subgroup="BaseCalled_template"
    ):
        path = "Analyses/{group}/{subgroup}/Fastq".format(
            group=basecall_group, subgroup=basecall_subgroup
        )
    ...
    def get_move(
        ...
        path = "Analyses/{group}/{subgroup}/Move".format(
            group=basecall_group, subgroup=basecall_subgroup
        )
    ...
    def get_trace(
        ...
        """
        path = "Analyses/{group}/{subgroup}/Trace".format(
            group=basecall_group, subgroup=basecall_subgroup
        )
        return np.asarray(self._read[path])
    ...
    def get_stride(self, basecall_group="Basecall_1D_000"):
        path = "Analyses/{group}/Summary/basecall_1d_template".format(
            group=basecall_group
        )
    ...
    def check_signallen_against_segmentation(
        self, segmentation_group="Segmentation_000"
    ):
        signal_len = len(self.get_raw_signal())
        assert signal_len == self.get_segmentation_start(
            segmentation_group
        ) + self.get_segmentation_duration(segmentation_group)

    def check_fastq_seqlen(
        self, basecall_group="Basecall_1D_000", basecall_subgroup="BaseCalled_template"
    ):
    ...
    def _get_scaling_of_a_read(self):
        ...
        try:
            channel_info = dict(
                list(self._read[global_key + "channel_id"].attrs.items())
            )
    ...
    def get_map_info(self, corrected_group, basecall_subgroup):
        alignment = self._read[
            "Analyses/" + corrected_group + "/" + basecall_subgroup + "/Alignment"
        ]
        ...
        for chrom_strand in grp_chrom_strands:
            chrom, strand = chrom_strand[0].decode("UTF-8"), chrom_strand[1].decode(
                "UTF-8"
            )
